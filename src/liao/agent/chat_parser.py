...
SYSTEM_TEXT_PATTERNS = [
    ...
    re.compile(
        r"^(Yesterday|Today|Tomorrow|昨天|前天|今天|星期[一二三四五六日天]|周[一二三四五六日天])$",
        re.IGNORECASE,
    ),
    ...
    re.compile(
        r"^(Read|Sent|Delivered|Typing|已读|已撤回|对方正在输入|消息已发送|以下为新消息)",
        re.IGNORECASE,
    ),
    re.compile(
        r"^\[(Image|Photo|Voice|Video|File|Location|Contact|Sticker|图片|语音|视频|文件|位置|名片|红包|转账)\]$",
        re.IGNORECASE,
    ),
    # --- app title / branding ---
    re.compile(
    ...
    re.compile(
        r"(撤回了一条消息|加入了?群聊|退出了?群聊|移出了?群聊|修改了群名|已添加了|邀请.*加入)",
        re.IGNORECASE,
    ),
    re.compile(
        r"(recalled a message|joined the group|left the group|removed from.*group)", re.IGNORECASE
    ),
...
class OCRChatParser:
    ...
    def parse_chat_area(
        self, window_info: "WindowInfo", chat_rect: tuple[int, int, int, int]
    ) -> list[ChatMessage]:
        ...
        img_w, _ = image.size
        messages = self._parse_ocr_results(results, img_w)

        print(f"    解析出 {len(messages)} 条消息")
        for m in messages:
            sender = "自己" if m.sender == "self" else "对方"
            print(f"      [{sender}]: {m.content[:50]}")

        return messages

    def _parse_ocr_results(
        self, results: list[tuple[list, str, float]], image_width: int
    ) -> list[ChatMessage]:
        ...
        for item in items:
            if (
                groups
                and (item["cy"] - groups[-1][-1]["cy"]) < 40
                and item["side"] == groups[-1][-1]["side"]
            ):
    ...
    def find_new_messages(
        self, current: list[ChatMessage], memory: "ConversationMemory"
    ) -> list[ChatMessage]:
