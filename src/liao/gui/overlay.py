...
class AreaSelectionOverlay(QWidget):
    ...
    def __init__(
        ...
        # Get DPI scale factor (macOS specific handling)
        screen = QApplication.primaryScreen()
        if sys.platform == "darwin":
            # On macOS, Qt coordinates are already in device pixels
            self._dpr = 1.0
        else:
    ...
    def paintEvent(self, event):
        ...
        if self._target_rect:
            ...
            painter.drawRect(
                QRect(wp.x(), wp.y(), int((t_right - t_left) / dpr), int((t_bottom - t_top) / dpr))
            )
        ...
        if event.button() == Qt.LeftButton and self._is_selecting:
            ...
            if self._start_global and self._current_global:
                ...
                if (right - left) > 30 and (bottom - top) > 30:
                    if sys.platform == "darwin":
                        # On macOS, use logical coordinates directly
                        # as Quartz window coords are already in screen pixels
                        self.area_selected.emit((int(left), int(top), int(right), int(bottom)))
                    else:
                        # Other platforms: apply DPI scaling
                        dpr = self._dpr
                        self.area_selected.emit(
                            (int(left * dpr), int(top * dpr), int(right * dpr), int(bottom * dpr))
                        )
                    self.close()
                else:
                    # Too small, reset
